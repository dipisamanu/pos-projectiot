...
- Argon2 per password e PIN carta
...
CORS_ORIGINS = (
    "*" if cors_raw == "*" else [x.strip() for x in cors_raw.split(",") if x.strip()]
)
...
db_pool = ThreadedConnectionPool(1, 30, **DB_CONFIG)
...
@contextmanager
def conn_cursor():
    """Context manager that yields a (conn, cur) tuple and ensures
    commit/rollback and connection release to the pool.
    """
    conn = db()
...
def is_access_token_revoked(jti):
    row = query(
        "SELECT id FROM revoked_access_tokens WHERE jti=%s LIMIT 1",
        (jti,),
        fetch=True,
        one=True,
    )
...
def token_richiesto(f):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            u = query(
                "SELECT id, attiva FROM utenti WHERE id=%s",
                (user_id,),
                fetch=True,
                one=True,
            )
...
def inject_user():
    return dict(
        ruolo=session.get("ruolo"), esercente_nome=session.get("esercente_nome")
    )
...
def dashboard():
    ...
    if session.get("ruolo") == "admin":
        return render_template(
            ...
            num_trans=query(
                "SELECT COUNT(*) AS n FROM transazioni WHERE esito='APPROVATA'",
                fetch=True,
                one=True,
            )["n"],
            num_ricar=query(
                "SELECT COUNT(*) AS n FROM ricariche WHERE stato='COMPLETATA'",
                fetch=True,
                one=True,
            )["n"],
            num_esercenti=query(
                "SELECT COUNT(*) AS n FROM esercenti", fetch=True, one=True
            )["n"],
        )
    num_proprie = query(
...
def utenti():
    ...
    if cerca:
        ...
        lista = query(
            "SELECT id, nome, username, iban FROM utenti ORDER BY nome", fetch=True
        )
...
def nuovo_utente():
    ...
    if request.method == "POST":
        ...
        if errore is None:
            if query(
                "SELECT id FROM utenti WHERE uid=%s", (uid,), fetch=True, one=True
            ):
                ...
                contatore = 1
                while query(
                    "SELECT id FROM utenti WHERE username=%s",
                    (username,),
                    fetch=True,
                    one=True,
                ):
                ...
                new_id = row["id"]
                query(
                    "UPDATE utenti SET iban=%s WHERE id=%s",
...
def qr_attivo(token):
    ...
    return render_template(
        "qr_attivo.html", ricarica=r, qr_b64=qr_b64, url_pagamento=token, token=token
    )
    ...
    return jsonify(
        {
            "stato": r["stato"],
            "data_completata": (
                r["data_completata"].strftime("%H:%M:%S")
                if r["data_completata"]
                else None
            ),
        }
    )
...
def pay_cliente(token):
    ...
    if not r:
        return render_template(
            "pay_cliente.html", errore="Token non valido", ricarica=None
        )
    if r["stato"] != "PENDING":
        return render_template(
...
def api_balance():
    u = query(
        "SELECT saldo FROM utenti WHERE id=%s", (g.user_id,), fetch=True, one=True
    )
    if not u:
        return jsonify({"success": False, "message": "Utente non trovato"}), 404
    stats = query(
        ...
        [
            {
                ...
                "category": (
                    r["categoria"] if r["categoria"] in CATEGORIE_VALIDE else "other"
                ),
            }
            for r in rows
        ]
...
def api_qr_confirm():
    ...
    try:
        with conn_cursor() as (conn, cur):
            cur.execute(
                "SELECT id, uid, nome, saldo, attiva FROM utenti WHERE id=%s FOR UPDATE",
                (g.user_id,),
            )
            u = cur.fetchone()
            if not u or not u["attiva"]:
                return (
                    jsonify(
                        {
                            "success": False,
                            "message": "Utente non valido o carta bloccata",
                        }
                    ),
                    403,
                )
            ...
            saldo_dopo = saldo_prima + importo
            cur.execute(
                "UPDATE utenti SET saldo=%s WHERE id=%s", (saldo_dopo, g.user_id)
            )
            cur.execute(
...
def api_transfer():
    ...
    try:
        with conn_cursor() as (conn, cur):
            ...
            # Impedisci bonifici a se stessi
            if dest_id == g.user_id:
                return (
                    jsonify(
                        {
                            "success": False,
                            "message": "Non puoi inviare un bonifico a te stesso, sullo stesso conto",
                        }
                    ),
                    400,
                )
            ...
            if dest_id:
                ...
                if not dest_row or not dest_row["attiva"]:
                    return (
                        jsonify(
                            {
                                "success": False,
                                "message": "Conto destinatario non disponibile o bloccato",
                            }
                        ),
                        400,
                    )
            ...
            if saldo_m_prima < amount:
                return (
                    jsonify({"success": False, "message": "Fondi insufficienti"}),
                    400,
                )
            ...
            # 3) Scala il mittente
            cur.execute(
                "UPDATE utenti SET saldo=%s WHERE id=%s", (saldo_m_dopo, g.user_id)
            )
            ...
            if dest_id:
                ...
                cur.execute(
                    "UPDATE utenti SET saldo=%s WHERE id=%s", (saldo_d_dopo, dest_id)
                )
                cur.execute(
                    """INSERT INTO transazioni (id_utente, uid_carta, nome_utente, titolo, tipo, categoria,
                       importo, saldo_prima, saldo_dopo, esito)
...
def not_found(_e):
    return (
        render_template("errore.html", codice=404, messaggio="Pagina non trovata"),
        404,
    )
